'''
Read the US population data from a CSV file, calculate the growth in population in consecutive years
and compute various statistical measures

Also creates two graphs - one showing the total population over the years and the other showing the change between
consecutive years
'''
...
def read_csv(filename):
    ...
    with open(filename) as f:
        ...
        for row in reader:
            # Eextract only the year from date
            year = row[0].split('-')[0]
            years.append(year)
            population.append(float(row[1]))
    # Reverse the list because the orginal data lists the mostt recent years first
    population.reverse()
    years.reverse()
...
def caculate_stats(population):
    ...
    print('Variance/Sd growth: {0:.5f}'.format(*variance_sd(growth)))
    return growth
...
if __name__ == '__main__':
    ...
    growth = caculate_stats(population)
